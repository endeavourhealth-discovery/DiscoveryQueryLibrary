...
if (filterType = 3) then  -- pivot over 6 months from pivot date (already set in cohort)
    ...
    cr.group_by,
	 ...
	 from cohort cr
	 join enterprise_pi.observation o on o.patient_id = cr.patient_id
     join
...
if (ignoreNullValues = 1) then
  delete from observationsFromCohort where result_value is null;
end if;

alter table observationsFromCohort add index (patient_id);
alter table observationsFromCohort add index (group_by);
