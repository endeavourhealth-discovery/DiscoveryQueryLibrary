...
DELIMITER //
CREATE PROCEDURE getObservations (
	IN codeSetId int,
	IN filterType int -- 0 earliest, 1 latest, 2 ever, 3 pivot around cohort.pivot_date (6 months)
)
BEGIN

call createObservationsFromCohort( codeSetId, filterType );
...
if (filterType = 0) then
	...
	create table filteredObservations as
		select distinct
		 ...
		 left join observationsFromCohort mcoo on mcoo.patient_id = mc.patient_id
