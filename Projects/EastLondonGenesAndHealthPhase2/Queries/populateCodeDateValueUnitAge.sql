...
DROP PROCEDURE IF EXISTS populateCodeTermDateValueUnitAgeV2;

DELIMITER //
CREATE PROCEDURE populateCodeTermDateValueUnitAgeV2 (
...
WHILE EXISTS (SELECT row_id from qry_tmp WHERE row_id > @row_id AND row_id <= @row_id + 1000) DO
   ...
   col, "Age = f.age_years - ( year(now()) - year(f.clinical_effective_date) ), ",
