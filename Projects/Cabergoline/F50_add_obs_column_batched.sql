...
(
	-- ASSUMES: Input dataset is called 'cohort'
...
BEGIN
	...
	inner join F50_cab_cohort_all_obs as obs_incl
		on ch.person_id = obs_incl.person_id
        and ch.patient_id = obs_incl.patient_id
		and obs_incl.snomed_concept_id in (',
		mixed_codes,
		')
	;'
	);
    ...
    WHILE row_offset < @T DO
        ...
        create temporary table batch as select person_id, organization_id, patient_id from cohort limit row_offset, batch_size;
        alter table batch add index (patient_id);
