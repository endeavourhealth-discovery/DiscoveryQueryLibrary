...
update dataset_wf d join ceg_compass_data.patient p on p.id = patient_id join
  ceg_compass_data.organization org on org.id = p.organization_id
  set PracticeODSCode = org.ods_code, PracticeName = org.name;
...
-- Age in years
update dataset_wf d join ceg_compass_data.patient p on p.id = patient_id
  join ceg_compass_data.person per on per.id = p.person_id
  set Age = per.age_years;

alter table dataset_wf add index patientIdIdx (patient_id);
