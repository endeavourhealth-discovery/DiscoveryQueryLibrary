...
BEGIN
	...
	SET max_event_log_date = DATE_ADD(current_event_log_date, INTERVAL 1 DAY); -- (SELECT dt_change FROM data_extracts.currentEventLogDate);
	...
	processLoop: WHILE (current_max_date <= max_event_log_date) DO
		...
		join data_extracts.filteredPatientsDeltaTest patients on patients.id = ev.record_id and ev.table_id = 2;
		...
		delete ev
		from data_extracts.evnt_tmp ev
		join data_extracts.filteredObservationsDeltaTest obs on obs.id = ev.record_id and ev.table_id = 11;
		...
		delete ev
		from data_extracts.evnt_tmp ev
		join data_extracts.filteredAllergiesDeltaTest obs on obs.id = ev.record_id and ev.table_id = 11;
		...
		delete ev
		from data_extracts.evnt_tmp ev
		join data_extracts.filteredMedicationsDeltaTest obs on obs.id = ev.record_id and ev.table_id = 11;
		...
		run_batch: WHILE EXISTS (SELECT row_id from qry_tmp WHERE row_id > @row_id AND row_id <= @row_id + 1000) DO
					  ...
					  REPLACE INTO data_extracts.filteredPatientsDeltaTest SELECT q.patientId FROM qry_tmp q WHERE q.row_id > @row_id AND q.row_id <= @row_id + 1000;
		...
		run_batch: WHILE EXISTS (SELECT row_id from qry_tmp WHERE row_id > @row_id AND row_id <= @row_id + 1000) DO
					  ...
					  REPLACE INTO data_extracts.filteredObservationsDeltaTest SELECT q.id, q.organization_id FROM qry_tmp q WHERE q.row_id > @row_id AND q.row_id <= @row_id + 1000;
					  SET @row_id = @row_id + 1000; 
		...
		run_batch: WHILE EXISTS (SELECT row_id from qry_tmp WHERE row_id > @row_id AND row_id <= @row_id + 1000) DO
					  ...
					  REPLACE INTO data_extracts.filteredAllergiesDeltaTest SELECT q.id FROM qry_tmp q WHERE q.row_id > @row_id AND q.row_id <= @row_id + 1000;
					  SET @row_id = @row_id + 1000; 
		...
		run_batch: WHILE EXISTS (SELECT row_id from qry_tmp WHERE row_id > @row_id AND row_id <= @row_id + 1000) DO
					  ...
					  REPLACE INTO data_extracts.filteredMedicationsDeltaTest SELECT q.id, q.organization_id FROM qry_tmp q WHERE q.row_id > @row_id AND q.row_id <= @row_id + 1000;
					  SET @row_id = @row_id + 1000; 
		...
		run_batch: WHILE EXISTS (SELECT row_id from qry_tmp WHERE row_id > @row_id AND row_id <= @row_id + 1000) DO
					  ...
					  REPLACE INTO data_extracts.filteredDeletionsDeltaTest SELECT q.record_id, q.table_id FROM qry_tmp q WHERE q.row_id > @row_id AND q.row_id <= @row_id + 1000;
					  SET @row_id = @row_id + 1000; 
		...
		update data_extracts.subscriber_extracts_test
